...
bool Sudoku::solve(int i, int j) {
    ...
    if (numbers[i][j] != 0) {
        // ultima posição, chegou ao fim do sudoku, retorna true
        if (i == 8 && j == 8)
            return true;
        else
            return solve(i + 1, j);  // resolve a proxima posição
    }

    // Estando numa box "0", verifica todos os numeros de 1 a 9 lá
    for (int n = 1; n <= 9; n++) {
        // se o numero não estiver na coluna, linha e caixa, marca essa tentativa com checkbox()
        if (!lineHasNumber[i][n] && !columnHasNumber[j][n] && !block3x3HasNumber[i / 3][j / 3][n]) {
            ...
            if (solve(i + 1, j))
                ...
                uncheckBox(i, j, n); // caso não tenha conseguido resolver quer dizer que com este numero,
                // qualquer combinação de numeros a seguir não dá soluções, logo volta a pôr a 0
            }
        }
    }
